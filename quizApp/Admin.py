...
import registerParticipant as regV
import adminFunction as adFunc
from tkinter import *
from registerParticipant import *
from adminFunction import *


def Home(root, frame1, frame3):  # home for admin
    root.title("Admin")
    for widget in frame1.winfo_children():
        widget.destroy()

    Button(frame3, text="Admin", command=lambda: Home(root, frame1, frame3)).grid(row=1, column=0)
    ...
    # Participant Login
    registerVoter = Button(frame1, text="Register Participant", width=15, command=lambda: regV.Register(root, frame1))

    # Show Answers
    showVotes = Button(frame1, text="Show Answers", width=15, command=lambda: adFunc.showResult(root, frame1))

    # Reset Data
    reset = Button(frame1, text="Reset All", width=15, command=lambda: adFunc.allReset(root, frame1))
